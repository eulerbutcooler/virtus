...
const (
	audienceAccess  = "virtus:access"
	audienceRefresh = "virtus:refresh"
)
...
func (s *AuthService) Refresh(ctx context.Context, refreshToken string) (*TokenPair, error) {
	claims, err := s.parseToken(refreshToken, audienceRefresh)
	if err != nil {
...
func (s *AuthService) ValidateAccessToken(tokenStr string) (*Claims, error) {
	claims, err := s.parseToken(tokenStr, audienceAccess)
	if err != nil {
		return nil, domain.ErrUnauthorized
	}
	return claims, nil
...
func (s *AuthService) issueTokenPair(ctx context.Context, user *domain.User) (*TokenPair, error) {
	...
	accessTok, _, err := s.mintToken(user, accessExpiry, audienceAccess)
	if err != nil {
		return nil, fmt.Errorf("mint access token: %w", err)
	}
	refreshExpiry := time.Now().Add(s.jwtCfg.RefreshTokenTTL)
	refreshTok, refreshJTI, err := s.mintToken(user, refreshExpiry, audienceRefresh)
	if err != nil {
		return nil, fmt.Errorf("mint refresh token: %w", err)
	}

	rtKey := rtCacheKey(user.ID, refreshJTI)
...
// Returns (signedToken, jti, error) so callers can use the jti without re-parsing.
func (s *AuthService) mintToken(user *domain.User, expiry time.Time, audience string) (string, string, error) {
...
func (s *AuthService) parseToken(tokenStr, expectedAudience string) (*Claims, error) {
	var claims Claims
